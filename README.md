...
起動すると115200bpsでHM3301を1秒間隔で計測し、1分ごとに大気環境側のPM1.0、PM2.5、PM10質量濃度の平均値と最大値を表示します。正常なフレームを受信するたびに青色LEDの状態が切り替わります。
...
確定した1分集計値は、PM1.0、PM2.5、PM10それぞれの平均値と最大値、および有効サンプル数を含む構造体として保持します。将来InfluxDBへ送信する場合は、この集計値をそのままPOST処理へ渡せます。
